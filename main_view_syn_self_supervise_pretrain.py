...
from network_syn import SphericalUnet
...
parser.add_argument('--start_decay', type=int, default=40,
...
# Load pretrain view synthesis network
view_syn_network = SphericalUnet()
view_syn_network = nn.DataParallel(view_syn_network)
view_syn_network.cuda()
view_syn_state_dict = torch.load('experiments/view_syn2/checkpoint_latest.pth')
load_partial_model(view_syn_network, view_syn_state_dict['state_dict'])
...
#------------------------------------
result_view_dir = 'view_syn_self_supervise'
if not os.path.exists(result_view_dir):
    os.makedirs(result_view_dir)   

# Train Function -------------------
def train(imgU, imgD, disp_true, mask, batch_idx):
    model.train()
    ...
    _, loss1 = compute_loss(imgD[:,:3,:,:], imgU[:,:3,:,:], output1, uvgrid, sgrid)
    _, loss2 = compute_loss(imgD[:,:3,:,:], imgU[:,:3,:,:], output2, uvgrid, sgrid)
    render_rgb, loss3 = compute_loss(imgD[:,:3,:,:], imgU[:,:3,:,:], output3, uvgrid, sgrid)
    ...
    '''    
    #--------------------------------------------------
    gt_render, _ = renderfromdisparity(imgD[:,:3,:,:], disp_true, uvgrid, sgrid)
    ...
    if batch_idx % 50 == 0:
        for i in range(2):
            up_img = up[i, :, :, :].transpose(1,2,0)
            down_img = down[i, :, :, :].transpose(1,2,0)
            pred_render_img = pred_render[i, :, :, :].transpose(1,2,0)
            gt_render_img = gt_render[i, :, :, :].transpose(1,2,0)
            disp_gt_img = cv2.normalize(disp_np[i,0,:,:],None,255.0,norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)
            disp_pred_img = cv2.normalize(disp_pred_np[i,:,:],None,255.0,norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)

            cv2.imwrite(result_view_dir + '/up_' + str(i) + '.png', up_img*255)
            cv2.imwrite(result_view_dir + '/down_' + str(i) + '.png', down_img*255)
            cv2.imwrite(result_view_dir + '/disp_' + str(i) + '.png', disp_gt_img)
            cv2.imwrite(result_view_dir + '/disp_pred_' + str(i) + '.png', disp_pred_img)
            cv2.imwrite(result_view_dir + '/pred_render_' + str(i) + '.png', pred_render_img*255)
            cv2.imwrite(result_view_dir + '/gt_render_' + str(i) + '.png', gt_render_img*255)
    loss.backward()
    optimizer.step()
...
def val(imgU,imgD,disp_true,mask,batch_idx):
    ...
    if batch_idx % 50 == 0:
        for i in range(2):
            ...
            cv2.imwrite(result_view_dir + '/test_up_' + str(i) + '.png', up_img*255)
            cv2.imwrite(result_view_dir + '/test_down_' + str(i) + '.png', down_img*255)
            ...
            cv2.imwrite(result_view_dir + '/test_pred_render_' + str(i) + '.png', pred_render_img*255)
            cv2.imwrite(result_view_dir + '/test_gt_render_' + str(i) + '.png', gt_render_img*255)
...
def main():
    ...
    for epoch in tqdm(range(start_epoch+1, args.epochs+1), desc='Epoch'):
        ...
        # Train --------------------------------------------------------------------------------------------------
        for batch_idx, (up, down, depth, depth_upmask, depth_downmask) in enumerate(train_dataloader):
            ...
            up, down, disparity = up.cuda(), down.cuda(), disparity.cuda()
            with torch.no_grad():
                up_pred = view_syn_network(down[:,:3,:,:])
                up_pred = torch.clamp(up_pred, 0, 1)
            up_pred = torch.cat([up_pred, up[:,3:,:,:]], 1)  
            loss = train(up_pred, down, disparity, depth_upmask, batch_idx)
            total_train_loss += loss
            global_step += 1
            if batch_idx % 5 == 0:
                print('epoch ', epoch, 'iter ', batch_idx, 'loss ', total_train_loss/(batch_idx+1))
        ...
        for batch_idx, (up, down, depth, depth_upmask, depth_downmask) in tqdm(enumerate(val_dataloader), desc='Valid iter'):
            ...
            depth_gt = disparity.data.cpu().numpy()
            val_output = val_output.data.cpu().numpy()
            mask_de_gt = depth_gt >0
            val_crop_rmse = np.sqrt(np.mean((val_output[mask_de_gt]-depth_gt[mask_de_gt])**2))
	        #-------------------------------------------------------------
            # Loss ---------------------------------
